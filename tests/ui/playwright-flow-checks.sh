...
assert_run "tree relationship forms persist selected kinds and statuses" \
  "${PWCLI}" run-code "async page => { await page.evaluate(() => { window.confirm = () => true; }); await page.locator('#tree-svg [data-id=\"tyler-000-0000-0000-000000000002\"]').first().click(); await page.locator('button[data-tree-sidebar-tab=\"relationships\"]').click(); const panel = page.locator('[data-tree-sidebar-panel=\"relationships\"]:not([hidden])'); await panel.waitFor(); const childCard = panel.locator('[data-tree-relationship-card=\"child\"]').first(); await childCard.getByRole('button', { name: 'Create and connect' }).click(); const childCreate = panel.locator('details[data-tree-relationship-group=\"child\"][open] [data-tree-create-form=\"child\"]').first(); await childCreate.locator('input[name=\"first_name\"]').waitFor({ state: 'visible' }); await childCreate.locator('input[name=\"first_name\"]').fill('Adoptive'); await childCreate.locator('input[name=\"last_name\"]').fill('Sprout'); await childCreate.locator('select[name=\"kind\"]').waitFor({ state: 'visible' }); await childCreate.locator('select[name=\"kind\"]').selectOption('adoptive'); await childCreate.locator('button[type=\"submit\"]').click(); await page.waitForFunction(() => { const panelEl = document.querySelector('[data-tree-sidebar-panel=\"relationships\"]:not([hidden])'); return panelEl && panelEl.textContent.includes('Adoptive Sprout'); }); const adoptiveText = await panel.textContent(); if (!adoptiveText || !adoptiveText.includes('Adoptive')) throw new Error('tree child create form did not persist adoptive kind'); await panel.locator('.tree-related-card', { hasText: 'Adoptive Sprout' }).first().getByRole('button', { name: 'Remove link' }).click(); await page.waitForFunction(() => { const panelEl = document.querySelector('[data-tree-sidebar-panel=\"relationships\"]:not([hidden])'); return panelEl && !panelEl.textContent.includes('Adoptive Sprout'); }); const partnerCard = panel.locator('[data-tree-relationship-card=\"partner\"]').first(); await partnerCard.getByRole('button', { name: 'Create and connect' }).click(); const partnerCreate = panel.locator('details[data-tree-relationship-group=\"partner\"][open] [data-tree-create-form=\"partner\"]').first(); await partnerCreate.locator('input[name=\"first_name\"]').waitFor({ state: 'visible' }); await partnerCreate.locator('input[name=\"first_name\"]').fill('Former'); await partnerCreate.locator('input[name=\"last_name\"]').fill('Flame'); await partnerCreate.locator('select[name=\"kind\"]').waitFor({ state: 'visible' }); await partnerCreate.locator('select[name=\"kind\"]').selectOption('domestic_partner'); await partnerCreate.locator('select[name=\"status\"]').selectOption('separated'); await partnerCreate.locator('button[type=\"submit\"]').click(); await page.waitForFunction(() => { const panelEl = document.querySelector('[data-tree-sidebar-panel=\"relationships\"]:not([hidden])'); return panelEl && panelEl.textContent.includes('Former Flame'); }); const partnerText = await panel.textContent(); if (!partnerText || !partnerText.includes('Separated')) throw new Error('tree partner create form did not persist partnership status'); await panel.locator('.tree-related-card', { hasText: 'Former Flame' }).first().getByRole('button', { name: 'Remove link' }).click(); await page.waitForFunction(() => { const panelEl = document.querySelector('[data-tree-sidebar-panel=\"relationships\"]:not([hidden])'); return panelEl && !panelEl.textContent.includes('Former Flame'); }); }"
