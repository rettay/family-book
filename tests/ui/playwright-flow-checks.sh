...
assert_run "tree nickname preference swaps node labels without changing canonical identity" \
  "${PWCLI}" run-code "async page => { await page.locator('#pref-show-names').check(); await page.locator('#pref-show-photos').check(); await page.locator('#pref-show-nicknames').check(); await page.locator('#save-tree-preferences').click(); await page.waitForTimeout(1200); const nodeText = await page.locator('#tree-svg [data-id=\"tyler-000-0000-0000-000000000002\"]').textContent(); if (!nodeText || !nodeText.includes('Ty Martin')) throw new Error('nickname preference did not preserve surname on node'); if (nodeText.includes('Tyler Martin')) throw new Error('nickname preference left formal name on node'); await page.locator('#pref-show-nicknames').uncheck(); await page.locator('#save-tree-preferences').click(); await page.waitForTimeout(1200); }"
...
assert_run "tree relationship forms persist selected kinds and statuses" \
  "${PWCLI}" run-code "async page => { await page.evaluate(() => { window.confirm = () => true; }); await page.locator('#tree-svg [data-id=\"tyler-000-0000-0000-000000000002\"]').first().click(); await page.locator('button[data-tree-sidebar-tab=\"relationships\"]').click(); const panel = page.locator('[data-tree-sidebar-panel=\"relationships\"]:not([hidden])'); await panel.waitFor(); const childCard = panel.locator('[data-tree-relationship-card=\"child\"]').first(); await childCard.getByRole('button', { name: 'Create and connect' }).click(); const childCreate = panel.locator('details[data-tree-relationship-group=\"child\"][open] [data-tree-create-form=\"child\"]').first(); await childCreate.locator('input[name=\"first_name\"]').waitFor({ state: 'visible' }); await childCreate.locator('input[name=\"first_name\"]').fill('Adoptive'); await childCreate.locator('input[name=\"last_name\"]').fill('Sprout'); await childCreate.locator('select[name=\"kind\"]').waitFor({ state: 'visible' }); await childCreate.locator('select[name=\"kind\"]').selectOption('adoptive'); await childCreate.locator('button[type=\"submit\"]').click(); const adoptiveCard = panel.locator('.tree-related-card', { hasText: 'Adoptive Sprout' }).first(); await adoptiveCard.waitFor(); const adoptiveText = await adoptiveCard.textContent(); if (!adoptiveText || !adoptiveText.includes('Adoptive')) throw new Error('tree child create form did not persist adoptive kind'); await adoptiveCard.getByRole('button', { name: 'Remove link' }).click(); await page.waitForFunction(() => { const panelEl = document.querySelector('[data-tree-sidebar-panel=\"relationships\"]:not([hidden])'); return panelEl && !panelEl.textContent.includes('Adoptive Sprout'); }); const partnerCard = panel.locator('[data-tree-relationship-card=\"partner\"]').first(); await partnerCard.getByRole('button', { name: 'Create and connect' }).click(); const partnerCreate = panel.locator('details[data-tree-relationship-group=\"partner\"][open] [data-tree-create-form=\"partner\"]').first(); await partnerCreate.locator('input[name=\"first_name\"]').waitFor({ state: 'visible' }); await partnerCreate.locator('input[name=\"first_name\"]').fill('Former'); await partnerCreate.locator('input[name=\"last_name\"]').fill('Flame'); await partnerCreate.locator('select[name=\"kind\"]').waitFor({ state: 'visible' }); await partnerCreate.locator('select[name=\"kind\"]').selectOption('domestic_partner'); await partnerCreate.locator('select[name=\"status\"]').selectOption('separated'); await partnerCreate.locator('button[type=\"submit\"]').click(); const partnerRelatedCard = panel.locator('.tree-related-card', { hasText: 'Former Flame' }).first(); await partnerRelatedCard.waitFor(); const partnerText = await partnerRelatedCard.textContent(); if (!partnerText || !partnerText.includes('Separated')) throw new Error('tree partner create form did not persist partnership status'); await partnerRelatedCard.getByRole('button', { name: 'Remove link' }).click(); await page.waitForFunction(() => { const panelEl = document.querySelector('[data-tree-sidebar-panel=\"relationships\"]:not([hidden])'); return panelEl && !panelEl.textContent.includes('Former Flame'); }); }"

assert_run "tree relationship cards support guarded removal after graph edits" \
  "${PWCLI}" run-code "async page => { await page.evaluate(() => { window.confirm = () => true; }); const tylerId = 'tyler-000-0000-0000-000000000002'; await page.locator('#tree-svg [data-id=\"' + tylerId + '\"]').first().click(); await page.waitForFunction((id) => document.querySelector('[data-tree-sidebar-person-id]')?.getAttribute('data-tree-sidebar-person-id') === id, tylerId); await page.locator('button[data-tree-sidebar-tab=\"relationships\"]').click(); const panel = page.locator('[data-tree-sidebar-panel=\"relationships\"]:not([hidden])'); await panel.waitFor(); await page.waitForFunction(() => { const panelEl = document.querySelector('[data-tree-sidebar-panel=\"relationships\"]:not([hidden])'); return panelEl && panelEl.textContent.includes('Playwright Relative'); }); const relCard = panel.locator('.tree-related-card', { hasText: 'Playwright Relative' }).first(); await relCard.scrollIntoViewIfNeeded(); await relCard.getByRole('button', { name: 'Remove link' }).click(); await page.waitForFunction(() => { const panelEl = document.querySelector('[data-tree-sidebar-panel=\"relationships\"]:not([hidden])'); return panelEl && !panelEl.textContent.includes('Playwright Relative'); }); }"
