...
assert_run "tree nickname preference swaps node labels without changing canonical identity" \
  "${PWCLI}" run-code "async page => { await page.locator('#pref-show-names').check(); await page.locator('#pref-show-photos').check(); await page.locator('#pref-show-nicknames').check(); await page.locator('#save-tree-preferences').click(); await page.waitForTimeout(1200); const nodeLabel = await page.locator('#tree-svg [data-id=\"tyler-000-0000-0000-000000000002\"]').getAttribute('data-render-label'); if (nodeLabel !== 'Ty Martin') throw new Error('nickname preference did not preserve surname on node: ' + nodeLabel); await page.locator('#pref-show-nicknames').uncheck(); await page.locator('#save-tree-preferences').click(); await page.waitForTimeout(1200); }"
