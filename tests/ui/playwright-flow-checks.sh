...
assert_run "tree relationships use searchable linking instead of full-family selects" \
  "${PWCLI}" run-code "async page => { await page.locator('button[data-tree-sidebar-tab=\"relationships\"]').click(); await page.locator('[data-tree-sidebar-panel=\"relationships\"]:not([hidden])').waitFor(); await page.locator('details[data-tree-relationship-group=\"child\"] summary').click(); const input = page.locator('details[data-tree-relationship-group=\"child\"] [data-tree-picker-input]').first(); await input.fill('Playwright Relative'); await page.locator('.tree-picker__result').getByText('Playwright Relative').click(); await page.locator('details[data-tree-relationship-group=\"child\"] button.btn--secondary').click(); await page.waitForTimeout(1200); const relCard = page.locator('.tree-related-card', { hasText: 'Playwright Relative' }).first(); if (!await relCard.count()) throw new Error('searchable relationship link did not persist'); await relCard.getByRole('button', { name: 'Open in tree' }).click(); await page.locator('.tree-sidebar-card__title').getByText('Playwright Relative').waitFor(); }"

assert_run "tree relationship cards support maintenance actions" \
  "${PWCLI}" run-code "async page => { await page.locator('#tree-svg [data-id=\"tyler-000-0000-0000-000000000002\"]').first().click(); await page.locator('button[data-tree-sidebar-tab=\"relationships\"]').click(); await page.locator('[data-tree-sidebar-panel=\"relationships\"]:not([hidden])').waitFor(); const relCard = page.locator('.tree-related-card', { hasText: 'Playwright Relative' }).first(); await relCard.getByRole('button', { name: 'Remove link' }).click(); await page.waitForTimeout(1200); const relText = await page.locator('[data-tree-sidebar-panel=\"relationships\"]').textContent(); if (relText && relText.includes('Playwright Relative')) throw new Error('relationship removal did not persist'); }"
