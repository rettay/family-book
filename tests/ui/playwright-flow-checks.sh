...
assert_run "tree sidebar summarizes selected family context and focus state" \
  "${PWCLI}" run-code "async page => { await page.locator('#tree-svg [data-id=\"tyler-000-0000-0000-000000000002\"]').first().click(); await page.locator('#tree-sidebar-focus-badge').waitFor({ state: 'hidden' }).catch(() => {}); const summary = page.locator('.tree-sidebar-context__summary'); const text = await summary.textContent(); if (!text || !text.includes('Robert Martin') || !text.includes('Yuliya Semesock') || !text.includes('Our Family')) throw new Error('selected-person summary missing expected relationships'); await page.locator('#tree-sidebar-set-focus').click(); await page.waitForTimeout(500); const focusStatus = await page.locator('#tree-focus-status').textContent(); if (!focusStatus || !focusStatus.includes('Tyler Martin')) throw new Error('focus status did not update after setting focus'); if (!await page.locator('#tree-sidebar-focus-badge').isVisible()) throw new Error('focus badge not shown for focused person'); }"
